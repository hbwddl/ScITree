...
To use this vignette, first set the starting directory on your computer. This vignette will create a directory for the simulation and a directory for the inference.

```{r}
## Run setup
## Necessary directories + simulation seed
base_dir <- "~/Documents/Research/Phylodynamics/ScTree/vignettes/"
random_seed <- 108

dirname <- paste0(base_dir,"sim_test_seed_",random_seed)
...
knitr::opts_knit$set(root.dir = dirname)
...
This vignette will walk you through an example simulation and analysis using the \ScITree\ package.  [https://www.biorxiv.org/content/10.1101/2024.11.20.624621v1](https://www.biorxiv.org/content/10.1101/2024.11.20.624621v1) 

# Simulation

A \ScITree\ simulation takes, as its input: 
- Geographic location data of individuals/premises
- Key epidemic parameters $\alpha, \beta, \kappa, a, b, c, d, \mu_1, \mu_2$, and $p$, as described in the \ScITree\ manuscript and in the format for the sim.param.key object.
- Auxiliary parameters for population size, random seed, length of genetic sequences, outbreak length, and other settings, as found in the sim.param.aux example.

The simulation generates, as its output:
- Epidemic data with time of exposure, time of infectiousness, and time of recovery/removal
- Transmission tree of who-infected-whom in the outbreak
- Genetic sequence data associated with transmission and sampling events

Now, we set the parameters for the simulation. These are epidemic parameters, plus the maximum x and y coordinates which we will use to generate the epidemic data.
...
We now generate our input epidemiological data, with columns for index/ID, X and Y coordinates, and our initial values for exposure time, infectious time, and removal time. If the time is meant to be unknown, fill in with 9,000,000.
...
The following parameters govern the number of MCMC iterations, plus the frequency of output.

```{r}
niter = 50 # Number of iterations
nfrequ = max(round(n_pop/3),1) # Number of individuals to update at each iteration
...
The following sets of parameters define the key scalar parameters and important auxiliary parameters which govern the simulation.
...
```
We have now simulated our outbreak, and we make a quick summary.
...
We now prepare to set up our inference. We will prepare the genetic data input to the inference from the simulated genetic sequences.

A ScITree inference takes a .FASTA file of sampled genetic sequences as input, with the names of the sequences denoting the individual's index number and the time of sampling.
...
We initialize the source of infection based on "available" infectious individuals at the initial time of exposure (simply assumed to be 1 day before the infectious time).
...
We now set up the parameters for the inference. The inference takes in several lists of settings:

1. Initial values for the scalar parameters, in the format of infer.param.key
2. Prior distribution parameters for scalar parameters, in the format of infer.param.priors
3. Standard deviations (scaling factors) for the normal proposal distributions for scalar parameters, in the format of infer.param.sf
4. Auxiliary parameters which control inference settings, in the format of infer.param.aux
...
We now create the .FASTA file with the sampled sequences.
...
We summarize the inference with several included R scripts.

The parameter_capture.R file examines the scalar parameter MCMC, the posterior_tree_capture.R file examines the identification of the correct source, and the bubble_plot.R file maps the percent correct posterior identification of infection sources.
