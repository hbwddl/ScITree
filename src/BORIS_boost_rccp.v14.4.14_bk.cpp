...
//
// #define source_known 1
// #define fix_bg_inf 0
//
// #define n_base_global 8000
//
...
//
// /* HANNAH ADDITION: POISSON DIST:*/
// typedef boost::math::poisson_distribution<double> poisson_mdist; //poisson_mdist(rate)
// typedef boost::math::binomial_distribution<double> binomial_mdist; // binomial_mdist(p,size)
// /* END HANNAH ADDITION */
//
// // - external parameters ----------------------------------------------------------------------------------------------
...
// };
//
// /* HANNAH ADDITION: COUNT THE NUMBER OF DIFFERENCES BETWEEN TWO SEQUENCES */
// int count_nb_diff(vector<int>& seq_1_arg, vector<int>& seq_2_arg){
//   int n_diff = 0;
//
//   for ( int i=0;i< seq_1_arg.size(); i++){
//     if(seq_1_arg[i] == 0 || seq_2_arg[i] == 0){
//       continue;
//     }
//     if(seq_1_arg[i] != seq_2_arg[i]){
//       n_diff = n_diff + 1;
//     }
//   }
//
...
// //--------------------------
//
// /* HANNAH ADDITION: BINOMIAL AND POISSON DRAW */
// int rbinom(int n, double p, rng_type& rng_arg);
// int rpoisson(double q, rng_type& rng_arg);
// /* END HANNAH ADDITION */
//
// //--------------------------
// /*- SIM header------------*/
...
// //typedef boost::exponential_distribution<double> Dexp;
//
// /* HANNAH ADDITION: BINOMIAL AND POISSON GENERATOR */
// typedef boost::binomial_distribution<int> Dbinom;
// typedef boost::poisson_distribution<int> Dpoisson;
// /* END HANNAH ADDITION */
//
// typedef boost::math::gamma_distribution<double> gamma_mdist;
// typedef boost::math::exponential_distribution<double> exp_mdist;
//
// /* HANNAH ADDITION: POISSON AND BINOMIAL MDIST */
// typedef boost::math::poisson_distribution<double> poisson_mdist;
// typedef boost::math::binomial_distribution<double> binomial_mdist;
// /* END HANNAH ADDITION */
//
...
// }
//
// /* HANNAH ADDITION: RBINOM AND RPOISSON */
// int rbinom(int n, double p, rng_type& rng_arg){
//   boost::variate_generator<rng_type &, Dbinom > rndm(rng_arg, Dbinom(n,p));
//   return rndm();
// }
//
// int rpoisson(double q, rng_type& rng_arg){
//   boost::variate_generator<rng_type &, Dpoisson > rndm(rng_arg, Dpoisson(q));
//   return rndm();
// }
// /* END HANNAH ADDITION */
//
...
// }
//
// /* HANNAH ADDITION: POISSON */
// inline double pdf_poisson_limit(double rate, int q) {
//   double pdf_poisson = 0.0;
//   pdf_poisson = pdf(poisson_mdist(rate),q);
//
//   if (pdf_poisson == 0) {
//     double dbl_min = std::numeric_limits< double >::min();
//     pdf_poisson = dbl_min;
...
// }
//
// /* HANNAH ADDITION: BINOMIAL */
// inline double pdf_binomial_limit(int size, double p, int n) {
//   double pdf_binomial = 0.0;
//   pdf_binomial = pdf(binomial_mdist(size,p),n);
//
//   if (pdf_binomial == 0) {
//     double dbl_min = std::numeric_limits< double >::min();
//     pdf_binomial = dbl_min;
//   }
//   return(pdf_binomial);
// }
//
// inline double surv_poisson_limit(double rate, double q) {
//   double surv_poisson = 0.0;
//   surv_poisson = 1 - cdf(poisson_mdist(rate), q);
//
//   if (surv_poisson == 0) {
//     double dbl_m = std::numeric_limits< double >::min();
//     surv_poisson = dbl_m;
...
//   //sf edit
//   if (opt_k80 == 0) {
//     p_1 = pdf_poisson_limit(abs_dt*mu_1,0)/n_base_global; // pr of a base not changing
//     p_2 = (1-p_1)/3; // pr of a transition of a base
//     p_3 = p_2*2;  // pr of a transversion (two possible events)
//   }
//   if (opt_k80 == 1) {
...
//
//   if (opt_k80 == 0) {
//     p_1 = pdf_poisson_limit(dt*mu_1_arg,0)/n_base_global; // pr of a base not changing
//     p_2 = (1-pdf_poisson_limit(dt*mu_1_arg,0))/(3*n_base_global); // pr of a transition of a base
//     p_3 = p_2*2;  // pr of a transversion (two possible events)
//   }
//   if (opt_k80 == 1) {
//     //K80: mu1 = alpha; mu2 = beta
...
//   double dt = t_2_arg - t_1_arg;
//
//   int nb_diff = count_nb_diff(seq_1_arg,seq_2_arg);
//
//   double pois_rate = dt*mu_1_arg;
//
//   double log_lh = log(pdf_poisson_limit(pois_rate, nb_diff));
//
//   return(log_lh);
//
// }
...
//
//   Rcout << "2892 " << log_lh_modified << "\t" << log_lh_current_arg << "\t" << acp_pr << "\n";
//
...
//     // HANNAH: KNOWN SOURCE
//     if(source_known == 1){
//       source = infected_source_current.at(xi_E_current.at(i));
//     } else if(fix_bg_inf == 1 && infected_source_current.at(xi_E_current.at(i)) == 9999){
//       source = infected_source_current.at(xi_E_current.at(i));
//     }
//
...
//
//   // /* HANNAH CHANGE: INITIALIZE WITH CORRECT INFECTED SOURCE SOURCE KNOWN */
//
//   if(source_known == 1 || fix_bg_inf == 1){
//
//     int epi_size = epi_final.infected_source.size();
//
//     for(int w = 0; w < epi_size; w++){
//       epi_final.infected_source.at(w) = atab_from.at(w);
//       infected_source_current.at(w) = atab_from.at(w);
//     }
//
//     /* Replace -99 in source with 9999 */
//     for(int i = 0; i < infected_source_current.size(); i++){
//       if(infected_source_current[i] < 0){
//         infected_source_current[i] = 9999;
//       }
//     }
//   }
//
...
//   for (int i = 0; i < (n_iter + 10); i++) { // mcmc loop mcmc main loop
//
//
//
//     if (opt_betaij >= 1) {
...
//
//     // HANNAH: NO UPDATE MU_2 AND P_BER
